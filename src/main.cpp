 ...
 //   SOLAR position
 SolarPosition my_position (49.551805, 20.566461); // Timbuktu, Mali, Africa
...
void setup() {
 ...
 for(int i=0; i<25;i++){
   for(int j=0;j<3;j++){
    ...
    delay(10);
    if (i<18){
      timePosArray[i][0] = map(timePosArray[i][0], 0, 254, 0, 1024);
    }
  ...
  azimuth_min = timePosArray[21][1]; 
  azimuth_max = timePosArray[21][2];
  azimuth_max = map(azimuth_max, 0, 254, 154, 410);
  servo_min = timePosArray[22][0];
  servo_max = map(timePosArray[22][1], 0, 254, 744, 1000);
}
...
void table_write()
{
  ...
  for (int row = 0;row < 18;row++){
    ...
    {   
      ...
      if (row<=18){
        EEPROM.write(eep_pos, map(timePosArray[row][0], 0, 1024, 0, 254));
        Serial.print("EEPROM WRITE POS: ");
        Serial.print(row);
        Serial.print("");
        Serial.println(eep_pos, map(timePosArray[row][0], 0, 1024, 0, 254));
      }
...
{ 
  ...
  int now_min = map (minute(), 0, 60, 0, 100);
  int now_hour = hour();
  ...
  result = map(result, 0, 254, 0, 1024);
  Serial.print("result: ");
...
void menu() {
  ...
  while (klik==0) {
    ...
    if (menu_count == 3){     
      ...
      timePosArray[21][1] = azimuth_min;
      timePosArray[21][2] = map(azimuth_max, 154, 410, 0, 254);
      timePosArray[22][0] = servo_min;
      timePosArray[22][1] = map(servo_max, 744, 1000, 0, 254);
      table_write();
